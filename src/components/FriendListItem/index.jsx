  ...
  render() {
    ...
    return (
        <div className="friendItem" key={id} ref={this.selfEle}>
          <img src={require(`../../headImgs/${headSculpture}.png`)} alt="no" className="headImg"/>
          <span className="score"><span>{score}</span>分</span>
          <img src={require(`../../levelIcon/level${computeLevel(score)}.png`)} alt="" className="levelImg"/>
          <span className="name">{name}</span>
          <span>注册时间：</span>
          <span>2019-12-24 12:20:15</span>
          <button className="delBtn" onClick={this.delHandle}>删除</button>
          <button className="matchBtn" onClick={this.contestHandle}>友谊赛</button>
    ...
    if (this.state.showFriendlyMatch) {
      return (
          <div className="friendlyMatchSettings">
            <button className="closeBtn" onClick={this.closeMatchSettings}>X</button>
            ...
            <button
            className="sendBtn"
                onClick={this.handleSendMatch}>发送</button>
          </div>
      )
    } else {
      return null;
